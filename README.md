...
Worker 用のエントリポイントを探しに行って失敗するため「ビルドコマンドなし」で保存してください。
...
- `PROJECT_NAME` : Cloudflare Pages のプロジェクト名（Repository Variables 推奨）。
  - GitHub の Settings > Secrets and variables > Actions > Variables で `PROJECT_NAME` を登録してください。
